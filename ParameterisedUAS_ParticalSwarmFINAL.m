...
[a,fval,exitflag,output] = particleswarm(@AircraftModel,length(parameter_Ranges),lb,ub,options)
...
if printresults && options == "ghost"
    ...
    % p01 = plot([100, 100], [1 2], 'r-');
    % p02 = plot([100, 100], [1 2], 'b-');
    % p03 = plot([100, 100], [1 2], 'k--');
    % ax2 = axes('Position', get(gca, 'Position'), 'Visible', 'off');
    % lgd2 = legend(ax2, [p01, p02, p03], {'UAS_1', 'UAS_2', 'Origional UAS'}, 'Location', 'southeast');
elseif printresults
    figure()
    patch(Wing1_X, Wing1_Y, 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'b'); hold on;
    patch(Tail1_X, Tail1_Y, 'b', 'FaceAlpha', 0.2, 'EdgeColor', 'b');
